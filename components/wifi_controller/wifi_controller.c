 ...
 *        Byte pertama dikunci agar valid standar IEEE 802.11 LAA (akhiran 2, 6, A, E),
 *        sedangkan 5 byte sisanya diacak total menggunakan hardware ESP32.
 */
void wifictl_set_random_laa_mac() {
    ...
    if (esp_wifi_get_mac(WIFI_IF_AP, current_mac) == ESP_OK) {
        ...
        current_mac[1] = esp_random() % 256;
        current_mac[2] = esp_random() % 256;
        ...
        if (err == ESP_OK) {
            ESP_LOGI(TAG, "LAA MAC sukses diacak (Valid) -> %02X:%02X:%02X:%02X:%02X:%02X",
